...
export function buildLayout(activities: ActivityInput[]): { nodes: CanvasNode[]; edges: CanvasEdge[] } {
  ...
  for (let ai = 0; ai < activities.length; ai++) {
    ...
    for (let si = 0; si < steps.length; si++) {
      ...
      for (let stI = 0; stI < stories.length; stI++) {
        ...
        if (story.estimate) data.estimate = story.estimate;
        if (story.status) data.status = story.status;
