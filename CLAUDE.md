...
- `frontend/src/components/panels/NodeContextBar.tsx` — Floating popup near selected node on both desktop and mobile (Edit/Delete/Comments buttons; viewers see only Comments)
- `frontend/src/components/panels/CommentsPanel.tsx` — Desktop right sidebar for card discussion threads + "Apply Changes via AI" button
- `frontend/src/components/panels/MobileCommentsPanel.tsx` — Mobile bottom sheet for card discussion threads
...
- **React effect cleanup race condition** (commit `2eb6928`): A `useEffect` with no dependency array runs after every render, so any Zustand state change (even clearing `pendingLayout` to `'none'`) causes a re-render → cleanup → timeout cancelled. The original ref-based capture pattern was still fragile because `setPendingLayout('none')` during render triggered re-renders that cancelled the 80ms timeout. Final fix: use a monotonic `layoutTrigger` counter in the store that increments alongside `pendingLayout`; the `useEffect` depends on `[layoutTrigger]` so only intentional layout requests trigger it, and unrelated re-renders don't cancel the timeout.
...
- **E2E `text=` selectors are fragile**: Playwright's `text=Edit Card` matches any element containing that text — adding a button with "Edit Card" elsewhere breaks tests relying on `text=Edit Card` for a heading. Use scoped selectors like `h3:has-text("Edit Card")` or `page.getByTestId('rf__wrapper').getByRole('button', { name: 'Edit' })` to avoid cross-component matches.
